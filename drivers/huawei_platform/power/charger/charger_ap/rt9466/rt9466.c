...
static int rt9466_pwr_rdy_irq_handler(struct rt9466_info *info)
{
	...
	dev_err(info->dev, "%s\n", __func__);
	mutex_lock(&info->pwr_rdy_lock);
	ret = rt9466_i2c_test_bit(info, RT9466_REG_CHG_STATC,
		RT9466_SHIFT_PWR_RDY, &pwr_rdy);
	mutex_unlock(&info->pwr_rdy_lock);
	if (ret < 0)
		return ret;
...
static int rt9466_check_vbus_hz_workaround(int disable)
{
	...
	while (1) {
		...
		if (((vbus_last > VBUS_THRESHOLD) && (adc_vbus > VBUS_THRESHOLD)) ||
			(vbus_last < VBUS_THRESHOLD) && (adc_vbus < VBUS_THRESHOLD))
			break;
	}
	return ret;
}
...
static void rt9466_aicl_work_handler(struct work_struct *work)
{
	...
	/* Enable MIVR IRQ */
	ret = rt9466_clr_bit(info, RT9466_REG_CHG_STATC_CTRL, RT9466_MASK_CHG_MIVR);
	if (ret < 0)
...
rt_9466_err_no_dev:
	...
	mutex_destroy(&info->pwr_rdy_lock);
