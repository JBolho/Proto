...
#define HISI_JPU_EMERG(msg, ...)    \
	do { if (hisi_jpu_msg_level > 0)  \
		pr_emerg("[hisijpu]%s: "msg, __func__, ## __VA_ARGS__); } while (0)
#define HISI_JPU_ALERT(msg, ...)    \
	do { if (hisi_jpu_msg_level > 1)  \
		pr_alert("[hisijpu]%s: "msg, __func__, ## __VA_ARGS__); } while (0)
#define HISI_JPU_CRIT(msg, ...)    \
	do { if (hisi_jpu_msg_level > 2)  \
		pr_crit("[hisijpu]%s: "msg, __func__, ## __VA_ARGS__); } while (0)
#define HISI_JPU_ERR(msg, ...)    \
	do { if (hisi_jpu_msg_level > 3)  \
		pr_err("[hisijpu]%s: "msg, __func__, ## __VA_ARGS__); } while (0)
#define HISI_JPU_WARNING(msg, ...)    \
	do { if (hisi_jpu_msg_level > 4)  \
		pr_warning("[hisijpu]%s: "msg, __func__, ## __VA_ARGS__); } while (0)
#define HISI_JPU_NOTICE(msg, ...)    \
	do { if (hisi_jpu_msg_level > 5)  \
		pr_notice("[hisijpu]%s: "msg, __func__, ## __VA_ARGS__); } while (0)
#define HISI_JPU_INFO(msg, ...)    \
	do { if (hisi_jpu_msg_level > 6)  \
		pr_info("[hisijpu]%s: "msg, __func__, ## __VA_ARGS__); } while (0)
#define HISI_JPU_DEBUG(msg, ...)    \
	do { if (hisi_jpu_msg_level > 7)  \
		pr_debug("[hisijpu]%s: "msg, __func__, ## __VA_ARGS__); } while (0)

//printk(KERN_DEBUG "[hisijpu]%s: "msg, __func__, ## __VA_ARGS__);

#define assert(expr) \
	if(!(expr)) { \
		pr_err("[hisijpu]: assertion failed! %s,%s,%s,line=%d\n",\
