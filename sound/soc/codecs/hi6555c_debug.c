		...
		size_t count, loff_t *ppos)
{
	...
	memset(kn_buf, 0, HI6555C_DBG_SIZE_CACHE);/* unsafe_function_ignore: memset */
		...
		size_t count, loff_t *ppos)
{
	...
	memset(kn_buf, 0, HI6555C_DBG_SIZE_WIDGET);/* unsafe_function_ignore: memset */
		...
		size_t count, loff_t *ppos)
{
	...
	memset(kn_buf, 0, HI6555C_DBG_SIZE_PATH);/* unsafe_function_ignore: memset */
...
static void hi6555c_debug_pop_debug(void)
{
	...
	for (idx_ps = 0; idx_ps < ps_index; idx_ps++) {
		...
		case HI6555C_POP_REG_WRITE:
			logi("w 0x%pK 0x%x\n", (void *)(unsigned long)(ps_cache[idx_ps].reg), ps_cache[idx_ps].val);
